...
class SoundDS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # the relative path
        audio_file = os.path.join(self.data_dir, self.files[idx])
        # Get the Class ID
        class_id = int(self.files[idx][:-4].split('-')[2])
        ...
        sgram = AudioUtil.spectro_gram(
            shift_aud, n_mels=20, n_fft=1024, hop_len=None)
        aug_sgram = AudioUtil.spectro_augment(
            sgram, max_mask_pct=0.1, n_freq_masks=2, n_time_masks=2)

        return aug_sgram.mean(dim=0).unsqueeze(dim=0), class_id

# print(SoundDS('./src')[0][0].shape)
